...
void calculatingWT(){
	...
	for(i = 0; i < nbProcess; i++){
		...
		serviceTime[i+1] = serviceTime[i] + burstTime[i];
		/* Recalculating serviceTime if CPU is idle while waiting for the next process arrive */
		if(serviceTime[i+1] < arrivalTime[i+1])
			serviceTime[i+1] += (arrivalTime[i+1]-serviceTime[i+1]);
		waitingTime[i] = serviceTime[i] - arrivalTime[i];
